...
--------------------
01e,04apr00,sjb  Remove include of aic7880.h.
01d,01feb00,djs  incorporate WRS review comments.
01c,07jun99,scb  Port from T1 to T2.
01b,05mar99,djs  changes made porting WRS pciAuotConfigLib.
01a,05mar99,djs  written based on pcPentium version 01h,19mar98,dat
*/
...
void sysAic7880PciInit(void)
    ...
    /*
     * two AIC 7880 PCI device IDs are supported; 1.) the 8178 ID is for
     * a 7880 device that is on a host adapter card, and 2.) the 8078 ID is
     * for a 7880 device that is on a motherboard.
     */

    if ((pciFindDevice (AIC7880_PCI_VENDOR_ID, AIC7880_PCI_DEVICE_ID,
			index, &busNo, &devNo, &funcNo)) != OK)
        {
	if ((pciFindDevice (AIC7880_PCI_VENDOR_ID, AIC7880_MB_PCI_DEVICE_ID,
			index, &busNo, &devNo, &funcNo)) != OK)
	    {
    ...
    pciConfigOutByte(busNo, devNo, funcNo, PCI_CFG_CACHE_LINE_SIZE, 8);
...
STATUS sysScsiInit (VOID)
    ...
    if ((pciIntConnect (INUM_TO_IVEC (((int) aicResource->irq) + INT_NUM_IRQ0),
