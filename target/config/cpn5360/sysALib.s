...
--------------------
01e,29apr02,dat  Rewritten from pcPentium/sysALib.s, ver 01z. only difference
		 is in sysReboot. (needed big changes in sysInit, and
		 sysCpuProbe).
01d,20mar00,djs  Initial code review changes.
01c,11feb00,dmw  made sysReboot() reset the CPU.
01b,30aug99,scb  reworked sysCpuProbe(), added sysCpuIdInst().
01a,30aug99,scb  written based on pcPentium version 01w,12may98,hdn
*/
...
FUNC_LABEL(sysReboot)
	movl	$ PIIX4_RC,%edx                /* reset control reg to edx */
	movl	$ PIIX4_RC_RCPU | PIIX4_RC_SRST,%eax /* reset value to eax */
	outb	%al,%dx
	jmp	sysReset0
sysReset0:
	ret
